(function(){
  ...
  function startRecording(){
    ...
    recorder.onstop=()=>{if(!chunks.length){status('Recording stopped without media data. Retry the recording.');return}mediaBlob=new Blob(chunks,{type:recorder.mimeType||'video/webm'});blobUrl=URL.createObjectURL(mediaBlob);const playback=$('recordingPlayback');playback.src=blobUrl;playback.hidden=false;$('cameraPreview').hidden=true;const script=$('teleprompter')?.textContent||'';window.LenderRecordingArtifact={blob:mediaBlob,mimeType:mediaBlob.type,size:mediaBlob.size,createdAt:new Date().toISOString(),approvedScript:script,requestedAspectRatio:'9:16'};$('downloadRecordBtn').disabled=false;$('discardRecordBtn').disabled=false;$('recordBtn').disabled=false;$('stopRecordBtn').disabled=true;cameraState('REAL RECORDING CREATED — review playback, discard, retry or download.');status(`Recorded source media exists (${Math.max(1,Math.round(mediaBlob.size/1024))} KB, ${mediaBlob.type}). Approved-script context and the 9:16 requirement remain associated with this artifact. Captions/branding still require processing and QA.`);if($('publishOutput'))$('publishOutput').textContent='RECORDED SOURCE MEDIA EXISTS — processing and independent QA required. This is not a published or completed video.'};
  ...
  function init(){
    ...
    $('selfModeBtn').addEventListener('click',()=>{$('enableCameraBtn').disabled=false;status('Self-record selected. Enable camera and microphone to begin a real capture.')},true);
