...
const click=async selector=>page.locator(selector).click({force:true});
...
await click('button:has-text("Save Employee Profile")');
...
for (const rule of [
  ...
  await click('button:has-text("Teach This Rule")');
}
...
for (const scenario of [
  ...
  await click('button:has-text("Send")');
...
await click('button:has-text("Load Robert Demo")');
...
await click('button:has-text("Add Lead")');
state=await page.evaluate(()=>JSON.parse(localStorage.getItem('ocg_leadflow_ai_pro_family_v3')));
...
await click('button:has-text("Add Activity")');
state=await page.evaluate(()=>JSON.parse(localStorage.getItem('ocg_leadflow_ai_pro_family_v3')));
...
for (const perm of ['draft','send','schedule']) await page.check(`[data-perm="${perm}"]`,{force:true});
await click('button:has-text("Save Permissions")');
await click('button:has-text("Create Follow-Up Draft")');
state=await page.evaluate(()=>JSON.parse(localStorage.getItem('ocg_leadflow_ai_pro_family_v3')));
assert.equal(state.actions[0].type,'draft');
assert.equal(state.actions[0].state,'ready');

await click('button:has-text("Queue Scheduling")');
state=await page.evaluate(()=>JSON.parse(localStorage.getItem('ocg_leadflow_ai_pro_family_v3')));
...
await click('button:has-text("Export AI DNA")');
