...
function incidents() {
  return `${header("Operational<br>incident command.", "Recovered release risks appear here. Live monitoring remains NOT CONNECTED and no real-time incident activity is simulated.", "INCIDENTS")}<section class="panel"><div class="list"><div class="row"><div><b>Commerce entitlement service is not connected</b><small>Paid routes are now fail-closed on this branch, but checkout, customer identity, entitlement grants and revocation are not implemented.</small></div><span class="status failed">P0 BLOCKER</span><div class="evidence">Owner: Security + Commerce<br>Safe state: fail closed</div><span>!</span></div><div class="row"><div><b>Commerce metrics require source verification</b><small>Hardcoded claims found in a sibling interface are not accepted as orders, revenue, conversion, fulfillment or webhook evidence.</small></div><span class="status review">DATA RISK</span><div class="evidence">Owner: Product Operations<br>Action: verify against provider</div><span>!</span></div></div><div class="alert" style="margin-top:16px"><b>Recovered evidence—not live telemetry</b><p>Sources: OCG LAB access-gate QA and repository inspection, 2026-09-01. Production monitoring and alert ingestion remain NOT CONNECTED.</p></div></section>`;
}
function commercial() {
  const gates = [
    ["Paid application boundary", "VERIFIED", "Anonymous protected-route matrix passed on preview deployment"],
    ["Canonical provider identity", "MISSING", "Whop is referenced; OCG LAB company/store identity is not recovered"],
    ["Product and checkout mapping", "MISSING", "No approved provider product/plan IDs recovered"],
    ["Customer authentication", "NOT CONNECTED", "No canonical customer identity boundary recovered"],
    ["Signed webhook", "NOT CONNECTED", "No endpoint or secret is claimed"],
    ["Entitlement store", "NOT CONNECTED", "No durable product-scoped grant/revocation record recovered"],
    ["Purchase and revocation QA", "MISSING", "End-to-end test purchase and cancellation evidence required"],
  ];
  return `${header("Build value.<br>Then capture it.", "Commercialization candidates reuse the existing OCG LAB catalog. Paid delivery remains fail-closed until provider, identity and entitlement evidence pass every gate.", "COMMERCIALIZATION")}<div class="grid-two commerce-layout"><section class="panel"><div class="panel-title"><h3>Commerce readiness</h3><span>1 OF ${gates.length} VERIFIED</span></div><div class="list">${gates.map(([name, state, evidence]) => `<div class="gate"><div><b>${name}</b><small>${evidence}</small></div><span class="status ${state === "VERIFIED" ? "verified" : state === "MISSING" ? "failed" : "review"}">${state}</span></div>`).join("")}</div></section><section class="panel"><div class="panel-title"><h3>Release decision</h3><span>NOT CONNECTED</span></div><div class="release-lock"><span>FAIL CLOSED</span><b>Do not deliver paid products until server-verified entitlement exists.</b><p>The architecture is defined without inventing credentials, infrastructure, revenue or live webhook activity.</p></div></section></div><section class="panel commercial-products"><div class="panel-title"><h3>Commercial product registry</h3><span>CANONICAL RECORDS</span></div>${productRows(products.filter((p) => /Commercial/.test(p.classification)))}</section>`;
}
const views = {
  home,
